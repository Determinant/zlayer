...
test('native sampled highs survive sparse contour intervals; nodata and outside corridor stay transparent', () => {
  ...
  values[0] = NaN;
  const result = paintTerrain(values, tile, [[a, b]], 1000, size);
  assert.equal(result.pixels[3], 0);
  assert.equal(result.pixels[4 * 20 + 3], 255);
  assert.equal(sampledHigh(values, tile, [[a, b]], size)?.elevation, 2743);
  assert.ok(paintTerrain(values, tile, [], 1000, size).pixels.every(value => value === 0));
...
test('contour fill has distinct 500/1000-foot bands and does not stack opacity at intersections', () => {
  ...
  const draw = (feet: number, segments = [segment]) => paintTerrain(new Float32Array(size * size).fill(feet), tile, segments, 500, size).pixels;
...
test('rendered fill keeps its core opacity and fades to transparent at eight nautical miles', () => {
  ...
  const pixels = paintTerrain(new Float32Array(size * size).fill(1200), tile, [segment], 1000).pixels;
  const nmPerPixel = nmPerWorldUnit((tile.y + 128.5 / size) / 512) / 512 / size;
  const alpha = (nm: number) => pixels[(128 * size + Math.floor(127.5 + nm / nmPerPixel)) * 4 + 1]!;
  assert.equal(alpha(0), 255, 'the numeric texture carries full corridor opacity in its low byte');
...
test('missing elevation only marks terrain incomplete where the rendered corridor has gaps', () => {
  ...
  assert.equal(paintTerrain(values, tile, [], 1000, size).incomplete, false);
  ...
  assert.equal(paintTerrain(interpolateElevation(coarse, 64, size), tile, [segment],
    1000, size).incomplete, true, 'interpolation spreading a gap into the corridor still warns');
});
