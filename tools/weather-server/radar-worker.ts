...
import { workerFailure } from './worker-protocol';
parentPort!.on('message', ({ raw, site, source, sourceHash, window }: { raw: ArrayBuffer; site: string; source: string; sourceHash: string; window: ScanWindow }) => {
  try {
    ...
    parentPort!.postMessage({ type: 'done', value: { body, scan: { site, source, sourceHash, observedAt: value.observedAt, bounds: value.bounds } } }, [body]);
