import { mkdir, open, readdir, readFile, rename, unlink, writeFile, type FileHandle } from 'node:fs/promises';
...
/** One atomic file per response. Only metadata stays in memory; refreshes share one promise. */
export class WeatherCache {
    ...
    for (const name of await readdir(this.options.directory)) {
      ...
      try {
        ...
        try {
          ...
          if (!(prepared || ['https://aviationweather.gov', 'https://nomads.ncep.noaa.gov', 'https://storage.googleapis.com'].includes(origin) &&
            ['json', 'package', 'index', 'range'].includes(entry.resource.kind)) ||
            ...
            stat.size !== size + 4 + entry.bytes || !/^[a-f0-9]{64}$/.test(entry.sha256)) throw new Error('Expired or invalid cache');
          ...
          entry = { ...entry, file, offset: size + 4, used: 0, removed: false };
        ...
        this.entries.set(entry.resource.key, entry); this.bytes += entry.bytes;
  ...
  retain(keys: Iterable<string>) { this.retained = new Set(keys); }
  ...
  async read(resource: Resource, maxAgeMs = resource.ttl): Promise<Payload | undefined> {
    ...
    if (entry) {
      ...
      try {
        const body = (await readFile(entry.file)).subarray(entry.offset);
  ...
  /** HTTP forecast reads only open saved files; they never enter the producer. */
  async open(resource: Resource): Promise<{ entry: Entry; handle: FileHandle } | undefined> {
    const entry = this.has(resource) ? this.entries.get(resource.key) : undefined;
    if (!entry) return undefined;
    try {
      const handle = await open(entry.file, 'r');
      try {
        if ((await handle.stat()).size !== entry.offset + entry.bytes) throw new Error('Truncated forecast file');
      } catch (error) { await handle.close(); throw error; }
      entry.used = (this.options.now ?? Date.now)();
      return { entry, handle };
    } catch {
      if (this.entries.get(resource.key) !== entry) return this.open(resource);
  ...
  private async save(resource: Resource, payload: Payload): Promise<void> {
    ...
    await this.trim(payload.body.length - (previous?.bytes ?? 0), previous ? 0 : 1, resource.key);
    const metadata = Buffer.from(JSON.stringify({ resource, status: payload.status, headers: payload.headers,
      checkedAt: payload.checkedAt, sha256: payload.sha256, bytes: payload.body.length }));
    ...
    try {
      await writeFile(temporary, Buffer.concat([prefix, metadata, payload.body]), { flag: 'wx', flush: true });
    ...
    this.entries.set(resource.key, entry); this.bytes += entry.bytes;
    if (previous) await this.remove(previous);
  }
