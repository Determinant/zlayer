...
and [contracts](contracts.md) for reference identities.
...
Chart, PDF and obstruction caches share the same receipt validation. Receipts are
trusted only from Cache Storage, never from network response headers. Obstruction
gzip files gain a receipt on acquisition or legacy-cache verification; later loads
reuse it while rebuilding and validating the spatial index.
...
from region lifecycle in `offline/`. `offline/bundle-repository.ts` exposes
...
[deployment readiness](deployment-readiness.md) for the required data-hosting setup.
