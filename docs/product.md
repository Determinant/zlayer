...
Stowing the AHRS toolbox offers **Stop**, **Background**, and **Cancel**, with
**Stop** as the prominent default. Stop ends motion sensing and recording, releases
the AHRS GPS lease, and clears calibration; reopening requires calibration.
Background explicitly keeps processing while stowed, with display updates paused.
Cancel leaves the toolbox open. A separate map GPS session is unaffected by Stop.

AHRS calibration must be able to complete without a GPS fix, including a session
that has never received one. It must tolerate small movements and cockpit vibration
during roughly steady, straight-and-level flight; perfect stillness is not required.
Larger motion and sustained turns still block calibration, with an explicit reason
instead of a completed progress indicator. Scrolling and temporary sensor pauses
preserve calibration progress: pause the timer, exclude missing readings, and resume
automatically when readings return. Before completing, averaged readings after the
pause must agree with the retained pose; a meaningful pose change replaces the old
reference while keeping the fresh readings. After calibration, current, uninterrupted
IMU readings keep the attitude indicator visible and moving. No fix or GPS below the aiding
...
return. Skip isolated bad readings and keep the sensor session running.
Incomplete calibration and estimator failures remain separate from these warnings. See the
[AHRS display policy](../src/layers/ahrs/README.md#calibration-and-validity) for
the state table, limits and recovery behavior.
