...
See [offline storage](offline-storage.md); an index alone does not make a region
available offline.
...
The service worker uses that complete URL as the Cache Storage key. It downloads the
whole MBTiles file once, verifies its byte length and SHA-256, and only then commits one
cache entry. Small packages use a full-file GET through this cache, then one reusable
...
eviction deletes the corresponding complete file from Cache Storage. Disk persistence
