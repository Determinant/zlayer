...
speed. When velocity is missing, the layer estimates it from a short sampling
...
The layer restarts a silent watch to request a fresh uncached position, since
...
and AHRS share one watch. Turning the map layer off or detaching the map clears the
watch, timers and motion samples only when no tool still holds a GPS lease. Stop AHRS
as well to stop location use. The GPS layer itself does not save position history;
optional [AHRS recordings](../src/layers/ahrs/recording.md) do include GPS fixes.
...
The product lives in `src/layers/ownship/`: `layer.ts` owns the watch and snapshot,
`position.ts` validates fixes and calculates geodesic motion, `geometry.ts` builds
map features, `map.ts` owns MapLibre resources, and `controls.tsx` owns controls
and status. The workspace registers it in the `ownship` rendering slot above
route and navigation labels. The map renderer remains lazy loaded.

## Release verification

The following is historical evidence, not a check of the current working tree.
The 2026-09-17 review found no additional GPS code blockers. `npm run verify`
passed all 452 unit tests, import/type checks and the production build. The 12
GPS browser tests passed against a production build in desktop Chrome. They cover
map rotation, the one-minute projection, missing velocity, poor accuracy, stale
fixes, denial/recovery, watch cleanup, map replacement, dateline crossings,
phone-size controls and cold offline launch. Native browser API tests use emulated
coordinates; visibility tests simulate suspension.

A separate fresh, headed Chrome session displayed the native location permission
prompt after enabling **GPS aircraft** in the full app, without pregranting
permission. Verify the release being served before testing the feature on the
HTTPS deployment; an existing installation may need to activate a new app version.
The local pass did not establish GPS availability on a production host. Follow
the [hosting and release checks](deployment-readiness.md) for that verification.
...
[graphics matrix](graphics-compatibility.md#run-the-checks) uses deterministic fixes
