...
Validated compressed bytes share the application's reference cache. Each worker
attachment revalidates the rolling manifest, with its validated saved copy as the
offline fallback. View and route changes reuse the loaded index. Disabling the
layer or unmounting releases the worker; subsequent attachment can
reuse the compressed cache. Failed loads remain retryable and are reported in
Layers. This opportunistic cache is separate from explicitly saved regional packs.
