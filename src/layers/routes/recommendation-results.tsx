...
export function RecommendationResults({ catalog, pair, navigation, airways, terminal, inset, onPreviewChange,
  ...
  const [stash, , retryStash] = useRouteStash();
  const model = useMemo(() => createRecommendationModel(history.data, preferred.data, pair, navigation, airways, terminal, stash.routes),
    [history.data, preferred.data, pair, navigation, airways, terminal, stash.routes]);
  ...
  return <>
    ...
    <RecommendationSection key={`frequency:${scope}`} id="frequency" title="Frequency" rows={model.groups.frequency} {...common}
      ...
      controls={<label className="route-history-filter"><span className="route-recommend-hint">Aircraft</span>
        <select className="ui-input ui-input--compact" aria-label="Aircraft" value={engine} onChange={event => { onPreviewInteraction(); setEngine(event.target.value); }}>
    ...
    {(['preferred', 'tec'] as const).map(category => <RecommendationSection key={`${category}:${scope}`} id={category}
      ...
      error={preferred.error} loading={preferred.loading} onRetry={preferred.retry}
    ...
    <RecommendationSection key={`stash:${scope}`} id="stash" title="Route Stash" rows={model.groups.stash} {...common}
      error={stash.error} loading={false} onRetry={retryStash}
      empty="No saved routes for this departure and destination." source="Saved on this device" />
  </>;
}
  ...
  return <section className="route-recommend-section" aria-labelledby={`route-${id}-title`} data-route-category={id}>
    ...
    {loading ? <p className="route-recommend-state" role="status">Loading {title.toLowerCase()} routes…</p>
      ...
      : <ul className="route-recommend-list">{rows.slice(0, limit).map(row => {
        ...
        const tecs = id === 'frequency' || id === 'stash' ? plan?.tecRoutes ?? [] : [];
        ...
        const label = id === 'stash' ? `${row.detail}: ${row.route}` : row.route;
        ...
        return <li key={row.id} className={`route-suggestion${selected ? ' is-selected' : ''}`} data-suggestion-id={row.id}>
          ...
          <button type="button" className="ui-button ui-button--compact route-suggestion-use" aria-label={`Use route ${label}`} disabled={!row.draft}
    ...
    {!loading && rows.length > limit && <button type="button" className="ui-button ui-button--compact ui-button--quiet route-recommend-more"
