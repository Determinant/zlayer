...
The table records the initial September 21 source audit. The later
[policy-5 maneuver review](approach-coverage.md#policy-5-maneuver-review)
records subsequent geometry results for the same edition; neither verifies a
later working tree or hosted deployment.
...
The [retained national report](../../../docs/evidence/chart-nav-review/2026-09-21/implementation/terminal-audit.json)
includes independent source/export recounts, source and implementation hashes,
and each choice with a review diagnostic. Its findings match the earlier audit;
the [earlier run's provenance](../../../docs/evidence/chart-nav-review/2026-09-21/implementation/README.md#earlier-terminal-audit)
is retained separately without duplicating the full report.
...
NASR ingestion requires the expected CSV headers, nonempty required national
tables and a matching edition on every row. Exported rows and explicit exclusions
must reconcile with the input; unavailable coordinates need a recorded exclusion,
while an absent coordinate column fails the build. Empty core navigation exports
must fail without replacing the previous generation. National totals vary by cycle.

Chart Supplement coverage comes from the FAA index independently of the books
available in a partial build. Missing applicable books or a missing supplement
snapshot prevent a regional download from being marked complete. An unrelated
missing region does not block a covered region, and partial catalogs remain usable
for browsing. Applicability is not inferred from all NASR airports having entries.
...
Saved synthetic fixes still require exact source membership and coordinates.

Publisher-to-client regression fixtures must reject missing required families,
same-count content mutations and legacy documents substituted for current
publications before caching or activation. Legacy formats have their own explicit
compatibility path. Wire-file hashes and parsed-JSON hashes identify different
representations; a client-captured hash alone cannot establish publisher intent.
Interrupted-publication checks must cover process exits as well as exceptions.

The [implementation evidence](../../../docs/evidence/chart-nav-review/2026-09-21/implementation)
