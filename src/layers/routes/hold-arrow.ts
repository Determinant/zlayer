...
export function holdArrowImage(color: readonly [number, number, number], outline: readonly [number, number, number]) {
  ...
  for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
    ...
    const fill = Math.max(0, Math.min(1, 4.5 - distance));
    const alpha = Math.max(0, Math.min(1, 7.5 - distance));
    data.set([
