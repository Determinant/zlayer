...
export function RouteRecommendations({ catalog, tokens, pins, onUseRoute, onPreviewChange = NO_PREVIEW }: {
  ...
  const { data, error, retry } = useRouteResource(open && canLoad && airports ? key : undefined, async () => {
    const { data, unavailable, airways, terminal, failed } = await loadRouteResources(catalog, 'recommendations');
    if (unavailable.includes('airports')) throw new Error('Airport data unavailable');
    return { navigation: data, airways, terminal, partial: failed };
  });
  ...
  return <>
    ...
    <button ref={button} type="button" className="ui-button ui-button--compact route-recommend" title={HINT}
    ...
    {open && <div ref={panel} id="route-recommendations" className="route-preview-panel route-recommendations"
      ...
      {error && <div role="alert"><p>{data
        ? 'Could not refresh recommendations. Previously loaded data remains available.'
        : 'Could not load recommendations. Connect to download the data, or retry your saved copy.'}</p>
        <p className="route-recommend-error">{error}</p>
        <button className="ui-button ui-button--compact" type="button" onClick={retry}>Retry</button></div>}
      {!canLoad ? <p role="status">For example, enter <strong>KSBA KSMO</strong> in the route box.</p>
        ...
        : !data ? !error && <p role="status">Loading FAA navigation data…</p>
        ...
        : <>
          ...
          {data.partial && !error && <div role="status"><p>Some navigation data is unavailable. Route details may be incomplete.</p>
            <button className="ui-button ui-button--compact" type="button" onClick={retry}>Retry navigation</button></div>}
        </>}
      </div>
    </div>}
  </>;
