...
export const Hsi = memo(function Hsi({ state, route, magneticModel }: {
  ...
  const heading = state.hsiHeading?.degrees ?? null;
  const headingValid = heading !== null && state.hsiHeading?.source === 'ahrs' && inertialLive &&
  ...
  const inertialAvailable = state.attitude !== null && Number.isFinite(state.attitude.yaw);
  const reference = heading ?? (inertialAvailable ? state.attitude!.yaw : null);
  ...
  const relative = heading === null && inertialAvailable;
  ...
  const magnetic = declination !== null;
  // The empty instrument defaults to magnetic heading; TRUE requires an actual reading.
  const trueReference = !magnetic && (heading !== null || state.position !== null || track !== null);
  ...
  const direction = relative ? `Relative direction ${degrees(reference)}. Heading unverified.`
    : reference !== null ? `${headingValid ? 'Heading' : 'Estimated heading'} ${degrees(bearing(reference))} ${referenceLabel}.` : '';
  const description = guidance
    ? `${warning ? `${warning}. ` : ''}${direction} Course ${degrees(bearing(guidance.course))} ${referenceLabel}, ${guidance.leg.from} to ${guidance.leg.to}. ${nm(guidance.distanceNm)} nautical miles to waypoint. ${side ? `${Math.abs(guidance.crossTrackNm).toFixed(2)} nautical miles ${side === 'R' ? 'right' : 'left'} of course.` : 'On course.'}${heading !== null ? ` ${headingValid ? 'True heading' : 'Estimated true heading'} ${degrees(heading)}.` : ''}${heading !== null && track !== null ? ` GPS track ${degrees(bearing(track))} ${referenceLabel}.` : ''}`
  ...
  return <section className="ahrs-hsi" aria-label="Horizontal situation indicator">
    <div className="ahrs-hsi-heading"><strong>HSI</strong><span>{relative ? 'IMU · REL'
      : `${state.hsiHeading?.source === 'gps' ? 'GPS/IMU' : 'IMU'} · ${trueReference ? 'TRUE' : 'MAG'}`}</span></div>
    <div className={`ahrs-hsi-readout${heading !== null || relative ? ' ahrs-hsi-heading-value' : ''}`}>
      {referenceName} {degrees(relative ? reference : bearing(reference))}{!relative && ` ${suffix}`}
    </div>
    <svg viewBox="90 18 140 146" className="ahrs-hsi-dial" role="img" aria-label={`HSI. ${description}`}>
      ...
      {(heading !== null || relative) && <g transform="translate(160 94) rotate(0)"
        ...
        <title>{relative ? `Relative direction ${degrees(reference)} — heading unverified`
          : `${headingValid ? 'True' : 'Estimated true'} heading ${degrees(heading)} T`}</title>
      ...
      {crossed && <g data-testid="hsi-invalid" aria-hidden="true">
    ...
    {(heading !== null || track !== null) && <div className="ahrs-hsi-reference">
    ...
    <dl className="ahrs-hsi-readings">
      <div><dt>{`DTK · ${suffix}`}</dt><dd className="ahrs-hsi-course">{guidance ? degrees(bearing(guidance.course)) : '—'}</dd></div>
      <div><dt>DIST · NM</dt><dd>{guidance ? nm(guidance.distanceNm) : '—'}</dd></div>
      <div><dt>XTK · NM</dt><dd className="ahrs-hsi-course">{guidance ? `${Math.abs(guidance.crossTrackNm).toFixed(2)} ${side}` : '—'}</dd></div>
      <div><dt>CDI SCALE</dt><dd>±{HSI_FULL_SCALE_NM} NM</dd></div>
    </dl>
    <div className="ahrs-hsi-target ahrs-hsi-course">
      {guidance ? `${guidance.from ? 'FROM' : 'TO'} ${guidance.leg.to}` : 'ROUTE GUIDANCE UNAVAILABLE'}
    </div>
