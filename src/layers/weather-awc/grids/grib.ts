...
export function decodeGrib(bytes: ArrayBuffer, expected: GribIdentity): GribField {
  ...
  const ids = section(1, 21), product = section(4, 34), g = section(3, 81), packing = section(5, 21), bitmap = section(6, 6), data = section(7, 5);
  ...
  const grid: LambertGrid = { width, height, latitude: angle(38), longitude: longitude(angle(42)),
    ...
    dx: g.getUint32(55) / 1000, dy: g.getUint32(59) / 1000, gridRelative: !!(g.getUint8(46) & 8),
    signature: [...new Uint8Array(bytes, g.byteOffset, g.byteLength)].map(n => n.toString(16).padStart(2, '0')).join('') };
  ...
  const bitmapBytes = new Uint8Array(bytes, bitmap.byteOffset + 6, bitmap.byteLength - 6);
  ...
  const bits = new Bits(raw.subarray(data.byteOffset + 5, data.byteOffset + data.byteLength));
  ...
  if (template === 0) {
    ...
    if (!groups) { for (let i = 0; i < count; i++) emit(0); }
      ...
      for (let i = 0; i < groups; i++) {
        const width = packing.getUint8(35) + bits.read(packing.getUint8(36));
      ...
      let total = 0;
      for (let i = 0; i < groups; i++) {
        const length = packing.getUint32(37) + bits.read(packing.getUint8(46)) * packing.getUint8(41);
        lengths[i] = i === groups - 1 ? packing.getUint32(42) : length; total += lengths[i]!;
      }
