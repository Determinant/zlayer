...
export function AirportWeather({ feature, client }: { feature: GeoPointFeature; client: MetarClient }) {
  ...
  return <Fragment key={featureKey(feature)}>
    <StationWeather feature={feature} client={client} name="METAR" intervalMs={METAR_REFRESH_MS}
      refreshStation={(id, signal) => client.refresh([id], signal)} View={MetarReportView} />
    <StationWeather feature={feature} client={tafClient} name="TAF" intervalMs={TAF_REFRESH_MS}
