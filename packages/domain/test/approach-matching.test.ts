...
import { approachIdent, findApproachRoute, findApproachRoutes, approachEntryOptions, createRouteResolver, routeDraftFromText } from '../src/index.js';
...
test('title formats retain the actual variant, operational qualifier and approach family', () => {
  for (const [airport, name, ident] of formats) {
    ...
    assert.equal(findApproachRoute(routes, airport, name)?.id, `${airport}:${ident}`, name);
  }
...
test('reviewed source exceptions expire with their data edition and never apply to a different variant or airport', () => {
  const future = structuredClone(routes);
  future.metadata.effectiveDate = '2026-10-01';
  for (const [airport, name, ident] of cases) {
    assert.equal(findApproachRoute(routes, airport, name)?.id, `${airport}:${ident}`, `${airport} ${name}`);
    assert.equal(findApproachRoute(future, airport, name), undefined, `new edition: ${airport} ${name}`);
    assert.equal(findApproachRoute(routes, 'KXXX', name), undefined);
  }
  assert.equal(findApproachRoute(routes, 'KSBD', 'ILS OR LOC Y RWY 06'), undefined);
  assert.equal(findApproachRoute(routes, 'KHGR', 'COPTER RNAV (GPS) RWY 09'), undefined);
  assert.equal(findApproachRoute(routes, 'KHGR', 'COPTER RNAV (GPS) RWY 27'), undefined);
  assert.equal(findApproachRoute(routes, 'KACY', 'COPTER ILS OR LOC/DME RWY 13'), undefined);
  assert.equal(findApproachRoute(routes, 'KEWR', 'COPTER ILS/DME RWY 22L'), undefined);
  assert.equal(findApproachRoute(routes, 'KHGR', 'RNAV (GPS) RWY 09')?.id, 'KHGR:R09');
  assert.equal(findApproachRoute(routes, 'KDFW', 'ILS RWY 18L (CONVERGING)'), undefined);
  const missing = structuredClone(routes);
  missing.procedures = missing.procedures.filter(p => p.id !== 'KDFW:I13RV');
  assert.equal(findApproachRoute(missing, 'KDFW', 'ILS V RWY 13R (CONVERGING)'), undefined, 'ordinary Z variant cannot substitute');
});

test('shared runway charts require both complete source routes to agree, including altitude constraints', () => {
  assert.equal(findApproachRoute(routes, 'KJFK', 'VOR OR GPS RWY 13L/R')?.id, 'KJFK:S13L');
  assert.equal(findApproachRoute(routes, 'KLAS', 'VOR RWY 26L/R')?.id, 'KLAS:S26L');
  assert.equal(findApproachRoute(routes, 'KBJC', 'VOR/DME RWY 30L/R'), undefined, 'BJC routes differ in altitude');
  assert.deepEqual(findApproachRoutes(routes, 'KBJC', 'VOR/DME RWY 30L/R').map(p => p.id), ['KBJC:D30L', 'KBJC:D30R']);
  const changed = structuredClone(routes);
  changed.procedures.find(p => p.id === 'KJFK:S13R')!.final[0]!.altitude!.first = '04000';
  assert.equal(findApproachRoute(changed, 'KJFK', 'VOR OR GPS RWY 13L/R'), undefined);
  changed.procedures = changed.procedures.filter(p => p.id !== 'KLAS:S26R');
  assert.equal(findApproachRoute(changed, 'KLAS', 'VOR RWY 26L/R'), undefined);
});
...
test('picker associations survive serialization and route expansion without losing the original chart title', () => {
  ...
  for (const [airport, name, ident] of [...cases, ...formats, ['KJFK', 'VOR OR GPS RWY 13L/R', 'S13L'],
    ...
    const p = findApproachRoutes(routes, airport!, name!).find(p => p.ident === ident)!, entry = approachEntryOptions(p)[0]!;
    ...
    const approach = { airportId: airport!, procedureId: 'original-chart', name: name!, cycle: '2609', entry: {
